# -*- coding: utf-8 -*-
"""ROS2 launch（由 ROS1 的 mapping_velody16.launch 迁移而来）。

ROS1 XML 与 ROS2 Python 的对应关系：
  <rosparam command="load" file="$(find point_lio)/config/velody16.yaml" />
      -> parameters=[os.path.join(pkg, 'config', 'velody16.yaml')]
...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        Node(
            ...
            output='screen',
            parameters=[os.path.join(pkg, 'config', 'velody16.yaml'), overrides],
        ),
        Node(
            package='rviz2',
            executable='rviz2',
