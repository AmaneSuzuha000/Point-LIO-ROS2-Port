...
void standard_pcl_cbk(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &msg) 
{
    ...
    if (to_sec(msg->header.stamp) < last_timestamp_lidar)
    {
    ...
    last_timestamp_lidar = to_sec(msg->header.stamp);
    ...
    if (con_frame)
    {
        if (frame_ct == 0)
        {
            time_con = last_timestamp_lidar; //to_sec(msg->header.stamp);
        }
        if (frame_ct < 10)
        {
    ...
    { 
        if (ptr->points.size() > 0)
        {
            ...
            time_buffer.emplace_back(to_sec(msg->header.stamp));
        }
    }
    }
...
void imu_cbk(const sensor_msgs::msg::Imu::ConstSharedPtr &msg_in) 
{
    ...
    msg->header.stamp = from_sec(to_sec(msg->header.stamp) - timediff_imu_wrt_lidar - time_lag_IMU_wtr_lidar);

    double timestamp = to_sec(msg->header.stamp);
...
bool sync_packages(MeasureGroup &meas)
{
    ...
    { 
        ...
        {
            double imu_time = to_sec(imu_deque.front()->header.stamp);
            ...
            while (imu_time < lidar_end_time)
            {
                ...
                imu_time = to_sec(imu_deque.front()->header.stamp); // can be changed
                imu_next = *(imu_deque.front());
            }
        }
        imu_pushed = true;
